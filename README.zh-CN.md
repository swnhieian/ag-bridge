...
- 再次同步时也会对已经 attach 的 live session 补跑历史对账，便于补回 reactive stream 临时中断后漏掉的 turn
- 通过 HTTP / SSE 输出文本、thinking、命令输出、审批请求和完整 step 详情
- 支持导出完整 session 数据
- 支持 CLI、Python SDK 和外部脚本接入
- 提供自动审批控制，适合半自动或全自动流程
- 即使 bridge 从 reactive streaming 回退到 polling fallback，自动审批也会继续生效
...
`POST /ag-sessions/sync` 适合周期性调用。除了把新发现的 AG 会话 attach 进 bridge，它也会对已经 attach 的 live session 再做一次历史补齐，帮助修复临时流中断时漏掉的后续 turn。
...
如果不传 `--base-url`，Node CLI 现在会按这个顺序解析 bridge：`AG_BRIDGE_URL`、本地 registry 的工作区 discovery、最后再回退到 `http://127.0.0.1:9464`。
...
- [更新记录](CHANGELOG.md)
- [构建与使用](docs/构建与使用.md)
- [构建、打包与安装](docs/build.md)
- [Node CLI 使用](docs/node-cli.md)
- [Python SDK 与 CLI](docs/python-cli.md)
- [接口文档](docs/接口文档.md)
- [设计方案](docs/设计方案.md)
...
在仓库里配置好 `OVSX_TOKEN` 这个 repository secret 之后，推送类似 `v0.1.3` 这样的 tag，就可以自动触发发布到 `Open VSX`。
