...
The output is written to `artifacts/cli/`, and now includes both the raw executable and a compressed archive, for example:

产物会输出到 `artifacts/cli/`，现在会同时包含原始可执行文件和压缩包，例如：
