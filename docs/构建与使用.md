...
### 3.1 Package the Node CLI as a Native Binary
...
The output is written to `artifacts/cli/`, for example:

产物会输出到 `artifacts/cli/`，例如：

```bash
artifacts/cli/ag-bridge-cli-darwin-arm64
```

The current implementation uses Node SEA, so it produces a host-specific binary for the machine that runs the packaging command.

当前实现基于 Node SEA，因此生成的是“执行打包命令的这台机器”对应的平台和架构专用二进制。

## 3.2 GitHub Actions and Open VSX Release
...
This binary is built for the current host platform and architecture.

这个二进制是按当前打包机器的平台和架构生成的。

### 7.2 Start the Local Service
...
### 7.4 List and Sync Native AG Sessions
